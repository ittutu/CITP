...
for all sorts. We can also define conditional equations `ceq true = false if ctor_i = ctor_j`
for all constructors, which will help CITP to find inconsistencies automatically.
...
We present this protocol both as an Observational Transition System
([here](#tas-specification-as-observational-transition-system)) and
as a specification with explicit states ([here](#tas-by-explicit-states)).
The specification
of the protocol and the proofs are available
[here](https://github.com/ittutu/CITP/tree/master/Examples/TAS).

#### TAS specification as Observational Transition System
...
We apply next a so-called proof strategy (a tactic list that usually produces
good results when applied to OTS) asking to apply the theorem of constants,
...
After applying this strategy the goals for init have been discharged and 17
goals have to be proven, being the first one:
...
We notice here that premise `2` corresponds to the current goal,
so we instantiate `I:X$Pid` with `I#3`, so we use premise `5`
to fulfill the condition of `2` and discharge the goal with command `init`:

```
(init 2 by I:X$Pid <- I#3 red)
```

This instantiation discharged 7 goals, so only 10 remain. The next one is:
...
In this case we can instantiate `1` with the substitution
`I:X$Pid <- I#3 ; J:X$Pid <- z#2`. In this way from premise `1`
we will have `I#3` equal to `z#2` but, from premise `3`, they should
be different:
...
In fact, we obtain the following module:
...
So we can reach a contradiction with premise `3` and the new premise `7`:

```
(. cp-l 3 >< 7)
```

We have to discharge 9 goals, being the next one:
...
sorts Bool Label Status Sys X$Pid .
op I#3 : -> X$Pid[metadata "new"].
op J#4 : -> X$Pid[metadata "new"].
op x#1 : -> Sys[metadata "induction-constant"].
op z#2 : -> X$Pid[metadata "induction-constant"].
eq I#3 ~ z#2 = false[metadata "3"].
eq J#4 ~ z#2 = false[metadata "4"].
eq lock(x#1)= close[metadata "7"].
eq pc(x#1,I#3)= cs[metadata "5"].
eq pc(x#1,J#4)= cs[metadata "6"].
ceq I#3 = z#2 if pc(x#1,z#2)= cs[metadata "8"].
ceq I:X$Pid = J:X$Pid if pc(x#1,I:X$Pid)= cs /\ pc(x#1,J:X$Pid)= cs[metadata "1" metadata "leave" nonexec].
ceq lock(x#1)= close if pc(x#1,I:X$Pid)= cs[metadata "2" metadata "leave" nonexec].
. . .
endfm,
...
From premise `1` we can obtain the current goal with the substitution
`I:X$Pid <- I#3 ; J:X$Pid <- J#4`, so we apply the corresponding command:

```
(init 1 by I:X$Pid <- I#3 ; J:X$Pid <- J#4 red)
```

And 8 goals are discharged, being the last one:
...
op x#1 : -> Sys[metadata "induction-constant"].
op z#2 : -> X$Pid[metadata "induction-constant"].
eq I#3 = z#2[metadata "3"].
eq J#4 ~ z#2 = false[metadata "6"].
eq lock(x#1)= open[metadata "5"].
eq pc(x#1,J#4)= cs[metadata "7"].
eq pc(x#1,z#2)= rs[metadata "4"].
ceq I:X$Pid = J:X$Pid if pc(x#1,I:X$Pid)= cs /\ pc(x#1,J:X$Pid)= cs[metadata "1" metadata "enter" nonexec].
ceq open = close if rs = cs[metadata "8"].
ceq z#2 = J#4 if rs = cs[metadata "9"].
ceq lock(x#1)= close if pc(x#1,I:X$Pid)= cs[metadata "2" metadata "enter" nonexec].
. . .
endfm,
eq z#2 = J#4[nonexec]; >
------------------- Current goal -------------------
...
which can be discharged from `2` with:

```
(init 2 by I:X$Pid <- J#4)
```

#### TAS as explicit states

In our specification of TAS with explicit states we can get rid of process
identifiers and directly work with labels, which stand for anonymous processes.
Hence, we define the sort `LABEL` as we did above:
...
And we define a multiset of labels, `LSet`, in the `LSET` module:
...
Note that the multiset is built with the `__` operator, that will
enable the decision procedures for multisets. We also define an operator
`allRS` for checking whether all the processes in the multiset are in the
remainder section:
...
The module `STATUS` is defined as we did above:
...
The module `TAS` defines the behavior of the module. We define the sorts
`Reach` for *reachable* states and `Sys` for all states. Terms of sort `Sys`
are pairs of a status (the lock) and a multiset of processes, represented by
their states:
...
We define the reachable states by means of membership axioms. In the initial
state (membersip axiom `ctor-init`) the lock is `open` and all the processes
are in the remainder (`rs`) section:
...
Note that the `metadata` attribute with a label starting by
`ctor-` is required for applying induction later. Next, we define
that a state with a process in the critical section (`cs`) is reachable
if previously we have a reachable state with a process in the remainder
section and an `open` lock:
...
Finally, the axiom `ctor-leave` specifies how processes leave the
critical section, changing the lock to `open`:
...
#### TAS properties - Explicit states

We prove the same properties that we presented above, but in this case
the formulation uses pattern matching for defining the states. In this
way, we state that reaching a state with a process in critical section
while the lock is `open` is impossible, so we use `true = false` in
the conclusion. Likewise, we indicate that it is impossible to have
two processes in the critical section at the same time (`cs cs` in the
second equation). Note that in both cases we use a matching condition
with a fresh variable `R` of sort `Reach`, that will be used for
induction:
...
We start the proof by applying induction, case splitting, and
implication:

```
(indx on R:Reach red cs red imp red)
```

This proof strategy just leaves 2 goals to be discharged, being
the first one:

```
--------------------- Goal: 1 ---------------------
< fmod TAS is
sorts Bool LSet Label Reach Status Sys .
subsort Label < LSet .
subsort Reach < Sys .
op LS#1 : -> LSet[metadata "new"].
op LS#2 : -> LSet[metadata "new"].
eq LS#1 = cs cs LS#2[metadata "2"].
eq allRS(LS#1)= true[metadata "1" metadata "init"].
. . .
endfm,
eq true = false ; >
------------------- Current goal -------------------
...
We note that `LS#1` contains 2 processes in the critical section, so
the normal form of `allRS(LS#1)` should be false, hence obtaining
`false = true` in premise `1`. To obtain this result we just use
`init` without a substitution:

```
(init 1 by )
```

This command discharges the 2 remaining goals, finishing the proof.
